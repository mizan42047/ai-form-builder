...
use AIFormBuilder\Config\ExtensionList;
...
class Extensions
{
    ...
    public function register_extension_assets()
    {
        ...
        foreach ($active_extensions as $slug => $extension) {
            $path = AIFORMBUILDER_EXTENSIONS_DIR . $slug;
            if (is_readable($path)) {
    ...
    {
        $active_extensions = ExtensionList::get_instance()->get_list('active');
        if (isset($block['blockName']) && str_contains($block['blockName'], 'ai-form-builder') && !empty($active_extensions)) {
